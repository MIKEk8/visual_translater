...
class TranslationOverlay:
    ...
    def show(self, text: str, position: Optional[Tuple[int, int]] = None) -> None:
        ...
        try:
            ...
            self._gui_queue.put({"type": "show_text", "data": {"text": text}})
    ...
    def update_text(self, text: str) -> None:
        """Update overlay text"""
        try:
            self._gui_queue.put({"type": "update_text", "data": {"text": text}})
    ...
    def set_position(self, x: int, y: int) -> None:
        ...
        try:
            ...
            self._gui_queue.put({"type": "set_position", "data": {"x": x, "y": y}})
    ...
    def _create_overlay_window(self) -> None:
        ...
        try:
            ...
            self._text_widget = type(
                "MockText",
                (),
                {
                    "configure": lambda **kwargs: None,
                    "delete": lambda start, end: None,
                    "insert": lambda _pos, _text: None,
    ...
    def _fade_in(self) -> None:
        ...
        try:
            if self.overlay_config.fade_animation:
                self._animate_fade(
                    start_alpha=0.0, end_alpha=self.overlay_config.opacity, duration=300
                )
        except Exception as e:
    ...
    def _fade_out(self) -> None:
        ...
        try:
            if self.overlay_config.fade_animation:
                self._animate_fade(
                    start_alpha=self.overlay_config.opacity, end_alpha=0.0, duration=300
                )
        except Exception as e:
        ...
        try:
            if hasattr(self, "_drag_start_x") and hasattr(self, "_drag_start_y"):
                ...
                if self.config.snap_to_edges:
                    new_x, new_y = self._snap_to_edges(
                        new_x, new_y, self.config.width, self.config.height
                    )
    ...
    def set_opacity_compat(self, opacity: float) -> None:
        """Compatibility opacity setter to avoid redefinition."""
        try:
            self.config.opacity = max(0.0, min(1.0, opacity))
            if self.overlay_window and hasattr(self.overlay_window, "attributes"):
